...
export class AuthController {
  ...
  async kakaoCallback(
    ...
    const newToken = this.authService.signToken(
      data['access_token'],
      data['expires_in'],
      'kakao',
    );
  ...
  async userInfo(
    ...
    switch (oauthAccessToken.provider) {
      case 'kakao':
        const kakaoData = await this.authService.kakaoUserInfo(
          oauthAccessToken.oauthAccessToken,
        );

        return {
          name:
            kakaoData.kakao_account.name ||
      ...
      case 'naver':
        const naverData = await this.authService.naverUserInfo(
          oauthAccessToken.oauthAccessToken,
        );
      ...
      case 'google':
        const googleData = await this.authService.googleUserInfo(
          oauthAccessToken.oauthAccessToken,
        );
